...
namespace XPBD
{
    public static class RotationalConstraint
    {
        ...
        public static float ApplyCorrection(float compliance, Vector3 corr, MyRigidBody body, MyRigidBody otherBody, bool velocityLevel = false)
        {
            //If no elongation
            ...
            float w_tot = GeneralizedInverseMass.Calculate(body, normal);

            if (otherBody != null)
            {
                w_tot += GeneralizedInverseMass.Calculate(otherBody, normal);
            }
            ...
            {
                // XPBD
                float alpha = compliance / body.dt / body.dt;
            ...
            UpdatePosAndRot(body, lambda_normal, velocityLevel);

            if (otherBody != null)
            {
                lambda_normal *= -1f;
                UpdatePosAndRot(otherBody, lambda_normal, velocityLevel);
            }
        ...
        //p is lambdanormal
        public static void UpdatePosAndRot(MyRigidBody body, Vector3 p, bool velocityLevel)
        {
            if (body.invMass == 0f)
            {
                return;
